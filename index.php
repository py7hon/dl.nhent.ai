...
header('Content-Disposition: inline; filename="Rick Astley - Never Gonna Give You Up.mp4"');
...
readfile('./.iae2013o0/Rick Astley - Never Gonna Give You Up.mp4');}
...
<meta charset="utf-8" />
<meta property="og:title" content="Download <?php echo $json_data2['title']['pretty'];?>" />
<meta property="og:url" content="https://dl.nhent.ai/dl/<?php echo $json_data2['id'];?>" />
<meta property="og:image" content="https://i.bakaa.me/galleries/<?php echo $json_data2['media_id'];?>/1.<?php echo $json_data3[1];?>" />
...
<body>
<center>
<img src="https://i.bakaa.me/galleries/<?php echo $json_data2['media_id'];?>/1.<?php echo $json_data3[1];?>" id="thumbnail" alt="thumbnail" class="img-thumbnail rounded" style="object-fit: contain; width:25%; flex: 0;">
...
<script type="text/javascript">"use strict";$(document).ready(function(){var $status=$("#status");function updateStatus(text,error){$status.text(text);if(error){$status.parent().addClass("text-danger");$progress.addClass("bg-danger")}else{$status.parent().removeClass("text-danger");$progress.removeClass("bg-danger")}}var urls=[<?php for ($i=1; $i <= $json_data2["num_pages"]; $i++){echo "'";echo "https://i.bakaa.me/galleries/";echo $json_data2['media_id'];echo"/";echo $i;echo ".";print $json_data3[$i];echo"'";echo ",";echo"\n";}?>];var nama="<?php echo $json_data2['title']['pretty'];?> ♥ nHentai";function updatePercent(percent){$("#progress_bar").removeClass("hide").find(".progress-bar").attr("aria-valuenow",percent).css({width:percent+"%"})}var zip=new JSZip();var count=0;var name=nama+".zip";urls.forEach(function(url){JSZipUtils.getBinaryContent(url,function(err,data){if(err){throw err}var filename=url.replace(/.*\//g,'');var img=zip.folder("<?php echo $json_data2['title']['pretty'];?>");img.file("Read Me FIRST.txt","Download from https://dl.nhent.ai/g/<?php echo $json_data2['id'];?> \n Dont Forget to join discord: https://discord.gg/y6kurbu and become to friend with admins https://fb.gg/composer.json").file(filename,data,{binary:true});count+=1;if(count==urls.length){zip.generateAsync({type:'blob'}).then(function(content){updateStatus("Success!");saveAs(content,name)})}})})});</script>
